  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center gap-3 mb-2 z-10">
        <div className="flex justify-center gap-2">
          {activeSteps?.map((_, index) => {
            ...
            return (
              <div key={index} className={`transition-all duration-500 ${isFilled ? "animate-heart-float" : ""}`} style={{ animationDelay: `${index * 0.15}s` }}>
                <Heart size={16} className={`transition-all duration-500 ${isFilled ? "fill-accent text-accent scale-110" : "text-accent/20 stroke-[2.5px]"} ${isCurrent && isRunning ? "animate-heart-pulse" : ""}`} />
              </div>
            );
          })}
        </div>
      ...
      <div className="relative flex flex-col items-center mb-6 z-10">
        {isRunning && (
          <>
            <Star size={12} className="absolute -top-6 -left-10 text-yellow-300 animate-sparkle" />
            <Star size={10} className="absolute top-12 -right-12 text-pink-300 animate-sparkle" />
            <div key={seconds} className="absolute -right-14 top-2 text-accent font-black text-sm animate-xp-float">+1 XP</div>
          </>
        )}
        <h1 className={`text-7xl lg:text-8xl font-black tracking-tighter text-appText mb-5 transition-all duration-500 font-timer ${isRunning ? "scale-110 drop-shadow-md" : ""}`}>
          {formatTime(seconds)}
        </h1>
        <div className="w-52">
          <div className="h-3 w-full bg-activeTaskBg rounded-full border border-containerBorder p-0.5 shadow-inner">
            <div className="h-full bg-accent rounded-full transition-all duration-1000 shadow-[0_0_15px_var(--accent)]" style={{ width: `${progressPercent}%` }} />
          </div>
      ...
      <div className="w-full mb-8 space-y-4 pt-4 z-10">
        ...
        {activeSteps?.[currentStepIndex + 1] && (
          <div className="px-6 py-4 bg-activeTaskBg/10 rounded-[30px] border border-dashed border-containerBorder/30">
            ...
            <p className="text-[13px] font-bold text-appText opacity-50 leading-relaxed text-left line-clamp-4">
              {activeSteps[currentStepIndex + 1].text}
            </p>
          </div>
        )}
      </div>
