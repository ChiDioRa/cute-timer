import { isRoutineDoneForToday } from '../utils/logicHelpers';

const DATABASE_ID = import.meta.env.VITE_NOTION_DATABASE_ID;
const BASE_URL = "/api/notion?endpoint=";

// --- 1. ТЕХНІЧНІ ДОПОМІЖНІ ФУНКЦІЇ ---
...
// --- 2. ПЕРЕТВОРЕННЯ ДАНИХ (Mapper) ---

const mapNotionPageToTask = async (page) => {
  ...
  // ✅ Виправлено: використовуємо хелпер для правильного статусу рутин
  const isCompleted = isRoutine 
    ? isRoutineDoneForToday(lastDoneStr, 5) 
    : page.properties["🌸"]?.checkbox || false;

  return {
    id: page.id,
    text: titleProp?.title[0]?.plain_text || "Без назви",
    completed: isCompleted, // ✨ ТЕПЕР ПРАВИЛЬНО
    isRoutine: isRoutine,
    lastDoneDate: lastDoneStr, // ✨ ДОДАНО
...
export const fetchNotionTasks = async () => {
  ...
  if (!data?.results) return [];
  return Promise.all(data.results.map(mapNotionPageToTask));
};
...
export const updateTaskStatusInNotion = async (pageId, isCompleted, dateIsoString) => {
  const properties = { "🌸": { checkbox: isCompleted } };
  ...
  return apiRequest(`/v1/pages/${pageId}`, "PATCH", { properties });
};
...
export const updateRoutineInNotion = async (pageId, count, dateIsoString) => {
  return apiRequest(`/v1/pages/${pageId}`, "PATCH", {
    properties: { 
      ...
      "Last Done Date": { date: { start: dateIsoString } }
    },
  });
};
